...
export default function NotificationsPage() {
  ...
  const loadNotificationSettings = async () => {
    try {
      const [settingsResponse, historyResponse] = await Promise.all([
        ...
        axios.get('/api/settings/notifications/history')
      ]);
      ...
      setNotificationHistory(historyResponse.data.history.map(notif => ({
        ...notif,
      ...
      toast({
        ...
        description: error.response?.data?.message || 'Could not load notification settings',
        variant: 'destructive',
      });
      setIsLoading(false);
    }
  ...
  const saveNotificationSettings = async (type, settings) => {
    try {
      const response = await axios.patch('/api/settings/notifications', {
        [type]: settings
      });
      
      switch(type) {
        case 'email':
          setEmailNotifications(response.data.email);
          break;
        case 'push':
          setPushNotifications(response.data.push);
          break;
        case 'slack':
          setSlackIntegration(response.data.slack);
          break;
        case 'webhooks':
          setWebhookConfig(response.data.webhooks);
          break;
        case 'quietHours':
          setQuietHours(response.data.quietHours);
          break;
      }
      ...
      toast({
        ...
        description: error.response?.data?.message || `Could not update ${type} settings`,
        variant: 'destructive',
      });
    }
  };
  ...
  const testWebhook = async (endpoint) => {
    try {
      await axios.post('/api/settings/notifications/test-webhook', { endpoint });
      toast({
      ...
      toast({
        ...
        description: error.response?.data?.message || 'Could not send test webhook',
        type: 'error',
      });
    }
  };
  ...
  const updateQuietHours = async (updates) => {
    try {
      ...
      toast({
        ...
        description: error.response?.data?.message || 'Could not update quiet hours',
        type: 'error',
      });
    }
  };
