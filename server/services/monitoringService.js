...
class MonitoringService {
  ...
  async getSystemMetrics() {
    try {
      ...
      // Get recent metrics from MongoDB (last 24 hours)
      ...
      // Calculate averages
      const responseMetrics = metrics.filter(m => m.metricType === 'response_time');
      const avgResponseTime = responseMetrics.length
        ? Math.round(responseMetrics.reduce((sum, m) => sum + Number(m.value || 0), 0) / responseMetrics.length)
        : 145;

      const errorMetrics = metrics.filter(m => m.metricType === 'error_rate');
      const errorRate = errorMetrics.length
        ? errorMetrics.reduce((sum, m) => sum + Number(m.value || 0), 0) / errorMetrics.length
        : 0.02;
  ...
  async getActiveAlerts() {
    try {
      const alerts = await Alert.find({ active: true, acknowledged: false })
        .sort({ createdAt: -1 })
        .limit(10)
        .lean();
